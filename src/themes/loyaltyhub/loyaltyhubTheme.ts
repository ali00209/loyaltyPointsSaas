 ...
 * LoyaltyHub Theme
 *
 * Fork of the Neutral theme (pure grayscale spine + OKLCH categorical
 * palette) rebranded to the LoyaltyHub identity: indigo-600 (#4F46E5)
 * accent, amber brand-gold (#FBBF24) logo mark, deep indigo (#312E81)
 * logo ink.
 *
...
import {loyaltyhubIconRegistry} from './icons';
...
export const loyaltyhubTheme = defineTheme({
  name: 'loyaltyhub',
  ...
  tokens: {
    ...
    // LoyaltyHub brand: indigo-600 #4F46E5 (light) / indigo-400 #818CF8
    // (dark). Muted = indigo-50 pastel (light) / indigo @ 24% (dark).
    '--color-accent': ['#4F46E5', '#818CF8'],
    '--color-accent-muted': ['#EEF2FF', '#4F46E53D'],
    '--color-neutral': ['#0000000F', '#FFFFFF1A'],

    // =========================================================================
    // Brand — LoyaltyHub identity tokens (logo mark, hero accents)
    //   brand-gold = amber-400 #FBBF24  (logo square)
    //   brand-ink  = indigo-900 #312E81 (star glyph on the logo)
    //
    // Custom tokens are not part of the closed TokenName union, but
    // `astryx theme build` emits them into loyaltyhub.css regardless.
    // ts-expect-error keeps the source file the single source of truth.
    // =========================================================================
    // @ts-expect-error custom token
    '--color-brand-gold': ['#FBBF24', '#FBBF24'],
    '--color-brand-ink': ['#312E81', '#312E81'],
    ...
    // Contrast: indigo accent takes white (L) / deep indigo (D) text
    '--color-on-accent': ['#ffffff', '#1E1B4B'],
  ...
  components: {
    ...
    progressbar: {
      ...
      'variant:accent': {
        // Indigo T50 saturated stop (LoyaltyHub brand accent)
        '--color-accent': '#4F46E5',
      },
      'variant:success': {
      ...
      // Loyalty tier variants — saturated stops from the categorical
      // palette (same sources as the categorical badges): used by the
      // Tier Distribution widget. Silver falls back to the built-in
      // neutral variant.
      'variant:orange': {
        // Orange T50 (source #d57113) / dark-ramp T70 #ffa258
        '--color-accent': 'light-dark(#d57113, #ffa258)',
      },
      'variant:yellow': {
        // Yellow T85 (source #f8c723) / dark-ramp T70 #deb433
        '--color-accent': 'light-dark(#f8c723, #deb433)',
      },
      'variant:purple': {
        // Purple T50 (source #980fb2) / dark-ramp T70 #f297ff
        '--color-accent': 'light-dark(#980fb2, #f297ff)',
      },
    },
